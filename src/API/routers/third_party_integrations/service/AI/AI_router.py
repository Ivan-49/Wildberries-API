...
from routers.third_party_integrations.service.AI.service.chat_gpt_api_client import GigaChatClient
...
async def analyze_product_history(
    ...
    try:
        ...
        if count > 100:
            raise HTTPException(status_code=400, detail="Превышено максимальное количество записей для анализа")
        products = await product_repository.get_latest_products_by_artikul(artikul, count, session)
        if not products:
            raise HTTPException(status_code=404, detail="Товар не найден")
