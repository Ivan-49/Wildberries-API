...
async def create_user_with_token():
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        ...
        login_response_by_username = await ac.post("/api/v1/auth/auth-by-username", data=login_payload_by_username)
        login_response_by_user_id = await ac.post("/api/v1/auth/auth-by-user-id", params=login_payload_by_user_id)
...
async def test_change_password(create_user_with_token):
    ...
    for _ in range(2):
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
            ...
            response = await ac.post('/api/v1/auth/change-password', params=payload,  headers = create_user_with_token['headers_jwt'] )
            assert response.status_code == 200
            data = response.json()
...
async def test_logout_user(create_user_with_token):
    ...
    async with AsyncClient(transport=ASGITransport(app=app),base_url="http://test" , headers=headers) as ac:
        ...
        assert response.status_code == 200
        assert response.json()['message'] == f"User {create_user_with_token['username']} logged out successfully"
